...
- **安全恢复**：恢复前执行只读预检和管理员复验；目录只恢复到新目标，数据库不覆盖非空目标。
...
| `mysqldump`、`mysql` | MySQL 逻辑备份与恢复 | 安装与目标数据库兼容的官方客户端；Shadoc 不自动安装数据库客户端 |
| `pg_dump`、`pg_restore` | PostgreSQL 逻辑备份与恢复 | 安装与目标数据库兼容的官方客户端；Shadoc 不自动安装数据库客户端 |

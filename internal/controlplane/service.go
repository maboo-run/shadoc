...
func manifestFromSnapshot(snapshot store.ControlPlaneSnapshotData) Manifest {
	...
	for _, item := range snapshot.Agents {
		manifest.Agents = append(manifest.Agents, AgentIdentity{ID: item.ID, RemoteHostID: item.RemoteHostID, ManagedInstallation: item.ManagedInstallation, AgentDataDir: item.AgentDataDir, CertificateSerial: item.CertificateSerial, CertificateNotAfter: item.CertificateNotAfter, Capabilities: append([]string(nil), item.Capabilities...), Status: item.Status, CreatedAt: item.CreatedAt, RevokedAt: item.RevokedAt})
	}

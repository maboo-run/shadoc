...
const (
	...
	windowsPrepareReenrollmentCommand = `powershell.exe -NoProfile -NonInteractive -Command "$ErrorActionPreference='Stop';$data=Join-Path $env:ProgramData 'shadoc-agent\data';New-Item -ItemType Directory -Force $data|Out-Null;& icacls.exe $data /inheritance:r /grant:r '*S-1-5-18:(OI)(CI)F' '*S-1-5-32-544:(OI)(CI)F'|Out-Null;if($LASTEXITCODE -ne 0){throw 'unable to protect Agent credentials'};New-Item -ItemType File -Force (Join-Path $data '.force-enrollment')|Out-Null;Remove-Item (Join-Path $data 'agent.crt'),(Join-Path $data 'agent.key'),(Join-Path $data 'ca.crt') -Force -ErrorAction SilentlyContinue"`
	windowsActivateCommand            = `powershell.exe -NoProfile -NonInteractive -Command "$ErrorActionPreference='Stop';$old=Join-Path $env:ProgramData 'restic-control-agent\data';$new=Join-Path $env:ProgramData 'shadoc-agent\data';$marker=Join-Path $new '.credential-migration';$force=Join-Path $new '.force-enrollment';if(!(Test-Path (Join-Path $new 'agent.crt'))){New-Item -ItemType Directory -Force $new|Out-Null;& icacls.exe $new /inheritance:r /grant:r '*S-1-5-18:(OI)(CI)F' '*S-1-5-32-544:(OI)(CI)F'|Out-Null;if($LASTEXITCODE -ne 0){throw 'unable to protect migrated Agent credentials'};New-Item -ItemType File -Force $marker|Out-Null;if((!(Test-Path $force)) -and (Test-Path (Join-Path $old 'agent.crt')) -and (Test-Path (Join-Path $old 'agent.key')) -and (Test-Path (Join-Path $old 'ca.crt'))){Copy-Item (Join-Path $old 'agent.crt'),(Join-Path $old 'agent.key'),(Join-Path $old 'ca.crt') $new}};$legacy=Get-Service restic-control-agent -ErrorAction SilentlyContinue;if($legacy){Set-Service restic-control-agent -StartupType Disabled;Stop-Service restic-control-agent -Force -ErrorAction Stop};try{& powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File (Join-Path $env:ProgramData 'shadoc-agent\install-service.ps1');if($LASTEXITCODE -ne 0){throw 'unable to activate Shadoc Agent'}}catch{if($legacy){Set-Service restic-control-agent -StartupType Automatic;Start-Service restic-control-agent};throw}"`
	windowsCleanupCommand             = `powershell.exe -NoProfile -NonInteractive -Command "$new=Join-Path $env:ProgramData 'shadoc-agent';$data=Join-Path $new 'data';Stop-Service shadoc-agent -ErrorAction SilentlyContinue;sc.exe delete shadoc-agent|Out-Null;if(Test-Path (Join-Path $data '.credential-migration')){Remove-Item (Join-Path $data 'agent.crt'),(Join-Path $data 'agent.key'),(Join-Path $data 'ca.crt'),(Join-Path $data '.credential-migration') -Force -ErrorAction SilentlyContinue};Remove-Item (Join-Path $data '.force-enrollment'),(Join-Path $new 'shadoc-agent.exe'),(Join-Path $new 'install-service.ps1'),(Join-Path $new 'enrollment.token'),(Join-Path $new 'ca.crt') -Force -ErrorAction SilentlyContinue;if(Get-Service restic-control-agent -ErrorAction SilentlyContinue){Set-Service restic-control-agent -StartupType Automatic;Start-Service restic-control-agent}"`
	linuxFinalizeCommand              = `legacy="$HOME/.config/systemd/user/restic-control-agent.service"; rm -f "$legacy.shadoc-migration" "$HOME/.local/share/shadoc-agent/.credential-migration" "$HOME/.local/share/shadoc-agent/.force-enrollment" "$HOME/.config/shadoc-agent/enrollment.token"`
	darwinFinalizeCommand             = `domain="gui/$(id -u)"; legacy="$HOME/Library/LaunchAgents/io.restic-control-agent.plist"; launchctl bootout "$domain" "$legacy" >/dev/null 2>&1 || true; rm -f "$legacy" "$legacy.shadoc-migration" "$HOME/.local/share/shadoc-agent/.credential-migration" "$HOME/.local/share/shadoc-agent/.force-enrollment" "$HOME/.config/shadoc-agent/enrollment.token"`
	windowsFinalizeCommand            = `powershell.exe -NoProfile -NonInteractive -Command "$legacy=Get-Service restic-control-agent -ErrorAction SilentlyContinue;if($legacy){Stop-Service restic-control-agent -Force -ErrorAction SilentlyContinue;sc.exe delete restic-control-agent|Out-Null;if($LASTEXITCODE -ne 0){throw 'unable to retire legacy Agent service'}};Remove-Item (Join-Path $env:ProgramData 'shadoc-agent\data\.credential-migration'),(Join-Path $env:ProgramData 'shadoc-agent\data\.force-enrollment'),(Join-Path $env:ProgramData 'shadoc-agent\enrollment.token') -Force -ErrorAction SilentlyContinue"`
...
func (r *Remote) PrepareReenrollment(ctx context.Context, platform Platform) error {
	...
	if _, err := r.runner.Run(ctx, command, nil); err != nil {
		return fmt.Errorf("prepare Agent re-enrollment: %w", err)
	}
	return nil
}

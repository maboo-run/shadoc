...
SELECT t.id,w.window_days,
	...
	CAST(ROUND(AVG(CASE WHEN r.status IN ('success','partial','failed') THEN ` + effectiveDuration + ` END)) AS INTEGER),
...
func (s *Store) loadTrendDays(ctx context.Context, now time.Time, indices map[string]int, trends []TaskTrend) error {
	...
	CAST(ROUND(AVG(CASE WHEN status IN ('success','partial','failed') THEN ` + effectiveDuration + ` END)) AS INTEGER),

...
The task editor keeps only the protection-scope summary, current rule count, and a **View and adjust protection scope** entry instead of repeating full rules and suggestions. That entry, or **Protection scope** on a task row, opens the task's dedicated source-inventory page; Back returns to the same task editor. The **Exclusion rules** tab combines per-line rule editing, current preview impact, and optional suggestions. Files are browsed one directory level at a time. A folder appears once for all of its descendants and is marked **Partially protected** when exclusions or unreadable items exist below it; open the folder to inspect the next level. Included, Attention, and Excluded views remain available, and search filters only the current level.

Files and folders can be staged for exclusion, while excluded entries can be staged for restored protection. **Preview rule effects** scans with the current draft and refreshes the summary, directory inventory, and rule impact in place. This creates only a 15-minute temporary preview and does not mutate the task. **Save rules** persists the rules only when they still match that preview; further edits, leaving the page, or preview expiry discard any unsaved effect. Restoring an item excluded by a wildcard removes the matching rule, so other content covered by that rule is restored too. Entry paths are relative to the task source; absolute source paths and file contents are not stored in the inventory. A scan is explicitly marked incomplete after 100,000 entries. New Agents upload bounded relative-path chunks over the existing mTLS channel, while older Agents keep the summary preview and show an upgrade notice for per-file browsing.

Directory run history reports **Files in scope** separately from **Files actually backed up or synchronized**. The first value is the number discovered and expected to be processed in that run; the second is the number successfully processed. When both values are available for the same Restic run and differ, Shadoc classifies the run as partial even if a snapshot was created. Equality without another error is required for complete success. Local and Agent tasks use the same rule; an Agent protects a partial snapshot before reporting it, and a protection failure pauses new backups to that repository until the next run completes the protection first.
...
If `restic` works in a terminal but initialization says it is missing, the macOS LaunchAgent or another background service usually did not inherit the terminal `PATH`. The control Service prefers the application-managed Restic, then checks the service `PATH` and common system locations; if none is available, install managed Restic from the Compatibility Center. Initialization errors distinguish a missing Restic executable from a repository-directory problem.

During Service startup, Shadoc resolves and probes local Restic, rsync, and database-client executable paths and caches the resulting compatibility report. The Compatibility Center, diagnostics export, and actual tasks reuse that result instead of probing again for each page. After installing or replacing a tool, choose “Detect local tools again” in the Compatibility Center; no Service restart is required. If the background service still cannot find a client on its `PATH`, expand “Specify tool paths manually” and enter an absolute path that passes identity and version validation. Save an empty value to return to automatic discovery. Manual MySQL paths become defaults for newly created or retested database connections, and a manual rsync path is used by subsequent local rsync runs. Remote Agent tools are still verified on their respective nodes.
